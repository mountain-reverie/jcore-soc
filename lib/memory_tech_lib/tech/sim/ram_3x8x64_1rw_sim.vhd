-- Models a 24x64 RAM with 1 read/write port.
...
architecture sim of ram_3x8x64_1rw is
  ...
  signal dr_tmp : std_logic_vector(23 downto 0) := (others => 'X');
...
begin
  ...
  begin
    ...
    if is_x(en) then
      assert false
        report "RAM 3x8x64 1rw EN UNKNOWN"
        severity WARNING;
    elsif is_x(wr) then
      assert false
        report "RAM 3x8x64 1rw WR UNKNOWN"
        severity WARNING;
    elsif is_x(we) then
      assert false
        report "RAM 3x8x64 1rw WE UNKNOWN"
        severity WARNING;
    elsif is_x(a) then
      assert false
        report "RAM 3x8x64 1rw ADDRESS UNKNOWN"
        severity WARNING;
    elsif is_x(dw) then
      assert false
        report "RAM 3x8x64 1rw DATAWRITE UNKNOWN"
        severity WARNING;
    else
  ...
  dr_pathdelay_gen: for i in 0 to 23 generate
    process(dr_tmp(i))
