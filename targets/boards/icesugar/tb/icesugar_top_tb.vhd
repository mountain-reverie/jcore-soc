...
begin
  ...
  pass_gate : process begin
    ...
    report "icesugar_top_tb PASSED: FROM SPRAM + SPRAM MEMTEST OK + W5500 programmed"
      severity note;
    done <= true;
    wait;
  end process;
  ...
  watchdog : process begin
    ...
    -- w5500_init_ping() runs; give it comfortable headroom (done is set the
    -- moment all markers hold, so a healthy run ends well before this).
    wait for 200 ms;
    assert done report "TIMEOUT: SPRAM MEMTEST OK / W5500 programmed not seen" severity failure;
    wait;
  end process;
end architecture;
