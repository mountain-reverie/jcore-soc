...
architecture sim of coremark_cosim_tb is
  ...
  procedure flash_slave(
    ...
    -- Deep power-down state. Real silicon powers up here as far as user logic
    -- is concerned: the iCE40 issues 0xB9 to the flash once configuration
    -- completes (Lattice TN1248), and until it sees 0xAB the flash ignores
    -- every command and drives nothing back. Modelling this is what makes the
    -- tb able to catch a reader that forgets the wake-up -- the previous
    -- model answered a cold 0x0B that real hardware silently ignores.
  ...
  begin
    loop
      ...
      if not awake then
        if cmd = x"AB" then
          awake := true;   -- takes effect on the CS rising edge below
        end if;
        miso <= '0';
        wait until cs_n = '1';
        next;
      ...
      assert cmd = x"0B"
        report "flash_slave: expected Fast-Read (0x0B), got 0x" &
               integer'image(to_integer(unsigned(cmd)))
        severity failure;
