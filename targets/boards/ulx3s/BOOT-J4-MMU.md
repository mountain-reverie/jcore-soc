...
## 2. Generate + compile the j4 device tree

`board.dts` is generated by socgen from the design; the `j4-rom` variant emits
`cpu@0 = "jcore,j4"` (the console/peripheral nodes are the same as j2-direct). Regenerate
for `j4-rom`, compile, then restore the committed default:

```sh
make ulx3s TARGET=soc_gen VARIANT=j4-rom        # board.dts now has cpu jcore,j4
dtc -I dts -O dtb -o /tmp/dt-j4.dtb targets/boards/ulx3s/board.dts
